/**
 * APP HOSTING ROUTES — Canvas-Studio SaaS Billing, Usage & Hosting Management
 *
...
router.get('/plan/:userId', requireAuth, async (req, res) => {
  try {
    ...
    const plan = await prisma.appHostingPlan.findFirst({
      where: { userId, status: 'active' },
      orderBy: { createdAt: 'desc' },
    });

    // No plan? Return inactive state — no free tier auto-creation
    if (!plan) {
      return res.json({
        success: true,
        plan: {
          ...
          currentApps: 0,
          daysRemaining: null,
          usagePercent: { bandwidth: 0, requests: 0, storage: 0, apps: 0 },
          tierConfig: null,
        },
      });
    }
...
router.get('/dashboard/:userId', requireAuth, async (req, res) => {
  try {
    ...
    const [plan, projects, deployments, usageRecords, subscriptions, canvasSubscriptions] =
      await Promise.all([
        ...
        // GenCraft Pro subscriptions (Canvas Studio)
    ...
    // Use the fetched plan or build an inactive default
    let hostingPlan = plan;
    if (!hostingPlan) {
      // No purchased plan — return inactive state (no free tier)
      hostingPlan = {
        id: null,
        userId,
        tier: 'none',
    ...
    // Billing summary
    const activeGencraftSub = subscriptions.find(
      (s) => s.status === 'active' && new Date(s.expiryDate) > new Date()
    );
    const activeCanvasSub = canvasSubscriptions.find(
      (s) => s.status === 'active' && new Date(s.expiryDate) > new Date()
    );
    const allSubs = [...subscriptions, ...canvasSubscriptions];
    const totalSpent = allSubs
    ...
    res.json({
      ...
      dashboard: {
        ...
        projects: projects.map((p) => ({
          ...
          latestDeployment: p.deployments[0] ? { ...p.deployments[0], bandwidthUsed: Number(p.deployments[0].bandwidthUsed || 0) } : null,
        ...
        deployments: {
          ...
          items: deployments.slice(0, 10).map((d) => ({
            ...d,
            bandwidthUsed: Number(d.bandwidthUsed || 0),
          })),
        },
        // Usage data
        usage: {
