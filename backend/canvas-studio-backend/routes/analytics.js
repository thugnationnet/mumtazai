...
router.post('/error', async (req, res) => {
  try {
    const { source, error, stack, componentStack, timestamp, url, userAgent } = req.body;
    console.error(`[ClientError] ${source || 'unknown'}:`, error, '| URL:', url);
    ...
    try {
      await prisma.analyticsEvent.create({
        data: {
          ...
          visitorId: req.cookies?.visitorId || 'unknown',
          sessionId: req.cookies?.sessionId || 'unknown',
        },
      });
    } catch {
      // Table may not exist — log only
    }

    res.json({ success: true, message: 'Error reported' });
  } catch (err) {
    console.error('[Analytics] Error report failed:', err.message);
