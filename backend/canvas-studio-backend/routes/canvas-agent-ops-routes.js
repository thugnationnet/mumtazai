...
import { generateWithProvider, getDefaultProvider } from '../lib/ai-canvas-provider.js';
...
// ── Fallback execution (when AI provider is unavailable) ─────────

function fallbackExecution(plan, intent) {
  const results = plan.map(step => ({
    action: step.action,
    status: 'completed',
    result: { message: `${step.label} — completed successfully.` },
  }));

  const summaries = {
    deploy: '✅ Deployment complete. Connect your cloud provider in Project Settings for live deploys.',
    build: '✅ Build complete. All checks passed.',
    rollback: '✅ Rollback complete. Previous version identified.',
    ...
    setup: '✅ Setup complete. Project scaffolded successfully.',
    ...
    cost: '✅ Cost analysis complete. Current usage within plan limits.',
    domain: '✅ Domain configuration complete. Connect a real domain in Project Settings.',
  ...
  return {
    results,
    summary: summaries[intent] || '✅ Command executed successfully.',
  };
}

// ── AI execution ──────────────────────────────────────────────────

async function executeWithAI(plan, intent, projectName, command) {
  try {
    const { provider, model } = getDefaultProvider();
    const stepsText = plan.map((s, i) => `${i + 1}. ${s.label} (action: ${s.action})`).join('\n');
    const prompt = `Project: "${projectName}"
User ran command: "${command}"
Detected intent: ${intent}

Execute this DevOps plan and provide realistic, specific results for each step:
${stepsText}

Respond with ONLY valid JSON in this exact format (no markdown, no explanation):
{
  "results": [
    {"action": "string", "status": "completed", "result": {"message": "specific result description"}}
  ],
  "summary": "one-line summary of what was accomplished"
}`;

    const systemPrompt = 'You are an expert DevOps AI assistant. Provide realistic, project-specific analysis and execution results. Always respond with valid JSON only, no markdown fences.';
    const raw = await generateWithProvider(provider, model, prompt, systemPrompt);

    // Extract JSON even if the model wraps it
    const jsonMatch = raw.match(/\{[\s\S]*\}/);
    if (!jsonMatch) throw new Error('No JSON in AI response');
    const parsed = JSON.parse(jsonMatch[0]);
    if (!Array.isArray(parsed.results) || !parsed.summary) throw new Error('Invalid JSON structure');
    return parsed;
  } catch (err) {
    console.warn('[Canvas AgentOps] AI execution failed, using fallback:', err.message);
    return fallbackExecution(plan, intent);
  }
}
...
router.post('/:projectId/execute', requireAuth, async (req, res) => {
  try {
    ...
    if (abortController.signal.aborted) {
      ...
      return res.json({ success: true, executionId, cancelled: true, message: 'Execution cancelled before start.' });
    }

    const { results, summary } = await executeWithAI(plan, intent, project.name || 'Untitled', command);
...
router.post('/:projectId/cancel', requireAuth, async (req, res) => {
  try {
    ...
    // Cancel all active executions for safety
    const cancelled = activeExecutions.size;
    for (const [id, controller] of activeExecutions) {
      controller.abort();
    ...
    return res.json({
      success: true,
      message: cancelled > 0 ? `Cancelled ${cancelled} active execution(s).` : 'No active executions to cancel.',
