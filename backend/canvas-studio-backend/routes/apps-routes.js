...
async function listDeployedApps(source, userId) {
  ...
  const prefixes = source
    ...
    : [`${S3_APPS_PREFIX}/canvas-studio/`, `${S3_APPS_PREFIX}/canvas-app/`, `${S3_APPS_PREFIX}/canvas-build/`, `${S3_APPS_PREFIX}/shared/`, `${S3_APPS_PREFIX}/`];
  ...
  for (const prefix of prefixes) {
    try {
      ...
      const metadataPromises = listResult.CommonPrefixes.map(async (pfx) => {
        ...
        if (!subdomain || ['canvas-studio', 'canvas-app', 'canvas-build', 'shared'].includes(subdomain)) return null;
        try {
