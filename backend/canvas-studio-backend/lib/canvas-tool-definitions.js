 ...
 * They use a shared tool format (input_schema).
 * OpenAI-compatible format is derived via getCanvasToolsForOpenAI().
 */

// =============================================================================
// TOOL DEFINITIONS (shared format — the source of truth)
// =============================================================================
...
export const CANVAS_TOOLS = [
  ...
  // ─── AI & ML TOOLS ────────────────────────────────────────────
  {
    ...
    description:
      'Multi-turn LLM conversations — route to xAI, Mistral, or OpenAI providers with temperature control and history.',
    input_schema: {
      type: 'object',
      properties: {
        messages: { type: 'array', description: 'Chat messages [{role, content}]' },
        model: { type: 'string', description: 'Model name (grok-3-mini, mistral-large-latest, gpt-4o, auto)' },
  ...
  },
  {
    name: 'image_ai',
    description:
      'AI-powered image understanding via vision model (GPT-4o). Actions: describe, analyze, extract_text, compare, classify, qa.',
    input_schema: {
      type: 'object',
      properties: {
        file: { type: 'string', description: 'Image path, filename, or URL' },
        action: { type: 'string', description: 'AI analysis type', enum: ['describe', 'analyze', 'extract_text', 'compare', 'classify', 'qa'] },
        options: { type: 'object', description: 'describe: {detail}. compare: {file_b}. qa: {question}. Optional: {provider: "openai"}.' },
      },
      required: ['file', 'action'],
    },
  },
...
- LLM chat: multi-turn conversations with xAI, Mistral, OpenAI, or auto-selected provider
...
// =============================================================================
//
// Canvas-studio panels: workspace | assistant | history | voice | image |
//                       deploy    | files     | null
//
// Each panel only gets the tools it actually needs so the LLM context window
// stays small and the model doesn't hallucinate tools that don't apply.
//
// Tool name prefixes used below:
//   canvas_file_*         — file system & project tree
//   canvas_terminal_run   — shell execution
//   canvas_build_*        — build / validate
//   canvas_project_*      — project settings, deps, env
//   canvas_deploy         — deployment
//   canvas_search_files   — project-wide search
//   canvas_archive_*      — ZIP / archive
//   canvas_dev_*          — developer tools (git, npm, docker, debug, test)
//   canvas_web_*          — web frontend tools
//   canvas_db_*           — database tools
//   canvas_api_*          — API & integration tools
//   canvas_webhook_listen, canvas_sdk_generate   — API extras
//   canvas_llm_*, canvas_ml_*                    — AI & ML tools
//   canvas_crypto_*, canvas_scan_*, canvas_auth_*, canvas_password_*, canvas_ssl_*  — security
//   canvas_agent_*        — agent intelligence / control
//   canvas_editor_select  — editor bridge
//   canvas_save_context, canvas_recall_context   — context memory
//   image_*, generate_image  — image generation (image panel)
//   canvas_markdown_*     — markdown tools (content)
//   canvas_analytics_*    — analytics/monitoring
//   canvas_log_parse, canvas_monitor_health, canvas_telemetry_send,
//   canvas_log_aggregate, canvas_metrics_collect  — monitoring
//   canvas_workflow_*, canvas_kg_*, canvas_growth_*, canvas_team_*,
//   canvas_llm_router…   canvas_data_*, canvas_geo_*, canvas_cloud_* — NOT USED
//                         by any panel — removed from all panel sets.

// Base tools every panel gets: file ops + terminal + project + build + search
const BASE_TOOLS = new Set([
  'canvas_file_create', 'canvas_file_edit', 'canvas_file_delete',
  'canvas_file_rename', 'canvas_file_read', 'canvas_file_copy',
  'canvas_list_directory', 'canvas_write_file', 'canvas_create_folder',
  'canvas_list_folders', 'canvas_file_exists', 'canvas_get_project_tree',
  'canvas_move_file', 'canvas_search_files',
  'canvas_terminal_run', 'canvas_build_validate',
  'canvas_project_set_framework', 'canvas_project_add_dependency', 'canvas_project_set_env',
  'canvas_agent_memory', 'canvas_agent_safety', 'canvas_agent_ui',
  'canvas_agent_control', 'canvas_editor_select',
  'canvas_save_context', 'canvas_recall_context',
]);

// Panel → additional tools on top of BASE_TOOLS
const PANEL_EXTRA_TOOLS = {
  workspace: new Set([
    // Full dev environment — all coding tools
    'canvas_dev_filesystem', 'canvas_dev_search', 'canvas_dev_intelligence',
    'canvas_dev_debug', 'canvas_dev_test', 'canvas_dev_git',
    'canvas_dev_npm', 'canvas_dev_docker',
    'canvas_web_analyze', 'canvas_web_scaffold', 'canvas_web_optimize',
    'canvas_web_transform', 'canvas_web_screenshot', 'canvas_web_lighthouse', 'canvas_web_scrape',
    'canvas_db_query', 'canvas_db_schema', 'canvas_db_backup',
    'canvas_db_migrate', 'canvas_db_analyze', 'canvas_db_connect',
    'canvas_api_request', 'canvas_api_mock', 'canvas_api_document',
    'canvas_api_test', 'canvas_api_transform', 'canvas_webhook_listen',
    'canvas_sdk_generate', 'canvas_api_validate', 'canvas_api_proxy', 'canvas_api_diff',
    'canvas_archive_core', 'canvas_archive_edit', 'canvas_archive_structure',
    'canvas_archive_bulk', 'canvas_archive_convert',
    'canvas_markdown_convert', 'canvas_markdown_validate', 'canvas_markdown_generate',
    'canvas_markdown_toc', 'canvas_markdown_format',
    'canvas_crypto_hash', 'canvas_crypto_encrypt', 'canvas_crypto_sign',
    'canvas_scan_secrets', 'canvas_scan_malware', 'canvas_auth_generate',
    'canvas_scan_dependency', 'canvas_password_audit', 'canvas_ssl_inspect',
    'canvas_agent_workflow', 'canvas_agent_delegate', 'canvas_agent_metrics',
  ]),

  assistant: new Set([
    // Chat assistant — coding tools only, no infra/image/deploy
    'canvas_dev_filesystem', 'canvas_dev_search', 'canvas_dev_intelligence',
    'canvas_dev_debug', 'canvas_dev_test', 'canvas_dev_git', 'canvas_dev_npm',
    'canvas_web_analyze', 'canvas_web_scaffold', 'canvas_web_optimize',
    'canvas_web_transform', 'canvas_web_screenshot',
    'canvas_api_request', 'canvas_api_document', 'canvas_api_test',
    'canvas_markdown_convert', 'canvas_markdown_generate', 'canvas_markdown_format',
    'canvas_agent_workflow', 'canvas_agent_delegate', 'canvas_agent_metrics',
  ]),

  files: new Set([
    // Files panel — file management + archive + search only
    'canvas_zip_files', 'canvas_unzip_files', 'canvas_file_watch', 'canvas_sync_files',
    'canvas_archive_core', 'canvas_archive_edit', 'canvas_archive_structure',
    'canvas_archive_bulk', 'canvas_archive_convert', 'canvas_archive_deploy',
    'canvas_dev_filesystem', 'canvas_dev_search',
  ]),

  image: new Set([
    // Image panel — image generation + basic file ops
    'image_create', 'image_transform', 'image_convert', 'image_compose',
    'image_filter', 'image_analyze', 'image_batch', 'image_background',
    'image_face', 'image_ai', 'image_export', 'image_ocr',
    'generate_image',
  ]),

  deploy: new Set([
    // Deploy panel — deployment + docker + build + security scan
    'canvas_deploy',
    'canvas_dev_docker', 'canvas_dev_npm', 'canvas_dev_git',
    'canvas_archive_deploy',
    'canvas_crypto_hash', 'canvas_scan_secrets', 'canvas_scan_dependency',
    'canvas_agent_workflow',
  ]),

  history: new Set([
    // History panel — read-only retrospective: no extra tools needed beyond base
  ]),

  voice: new Set([
    // Voice panel — conversational only, minimal tools
    'canvas_markdown_convert', 'canvas_markdown_generate',
  ]),
};

// Cache per panel to avoid recomputing
const _panelToolCache = new Map();

/**
 * Return the tool array for a specific canvas-studio panel.
 * Falls back to workspace (full set) if panel is unknown.
 */
