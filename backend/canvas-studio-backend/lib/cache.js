...
class CacheManager {
  ...
  async init() {
    ...
    try {
      // Try Redis first — uses keyPrefix to namespace all keys for this backend
      ...
      this.client.on('connect', () => {
        console.log(`✅ Redis cache connected (prefix: "${redisPrefix || 'none'}") to:`, redisUrl.replace(/:[^:@]+@/, ':***@'));
  ...
  async clear(pattern = '*') {
    ...
    try {
      if (this.client && this.isConnected) {
        // ioredis keyPrefix is NOT applied to KEYS command, so add it manually
        // But DEL DOES auto-prepend keyPrefix, so strip it from found keys
        const prefix = this.client.options.keyPrefix || '';
        const prefixedPattern = prefix + pattern;
        const keys = await this.client.keys(prefixedPattern);
        if (keys.length > 0) {
          // Strip prefix from keys because del() will re-add it
          const strippedKeys = prefix ? keys.map(k => k.startsWith(prefix) ? k.slice(prefix.length) : k) : keys;
