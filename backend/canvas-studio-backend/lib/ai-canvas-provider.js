 ...
 * Providers: xAI (default), Mistral (fallback), OpenAI (fallback)
 */
 ...
 * Priority: xAI (default) > Mistral (fallback) > OpenAI (fallback)
 */
export function getDefaultProvider() {
  const providers = [
    { name: 'xai', key: 'XAI_API_KEY', model: process.env.XAI_DEFAULT_MODEL || 'grok-3-mini' },
    { name: 'mistral', key: 'MISTRAL_API_KEY', model: process.env.MISTRAL_DEFAULT_MODEL || 'mistral-large-latest' },
    { name: 'openai', key: 'OPENAI_API_KEY', model: process.env.OPENAI_DEFAULT_MODEL || 'gpt-4o' },
  ];
