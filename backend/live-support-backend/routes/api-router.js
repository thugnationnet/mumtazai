/**
 * LIVE SUPPORT BACKEND — API ROUTER (port 3012)
 * Mounts: support, live-support, contact
...
import liveSupportRouter from './live-support-routes.js';
...
router.get('/health', (_req, res) => {
  res.json({
    success: true,
    message: 'Live Support API is healthy',
...
// ── Support (legacy ticket routes from backend) ────────────────

router.use('/support', apiLimiter);
router.use('/support', supportRouter);

// ── Live Support (AI chat + ticket management) ─────────────────

router.use('/live-support', apiLimiter);
router.use('/live-support', liveSupportRouter);
...
router.use((req, res) => {
  res.status(404).json({
    success: false,
    message: 'Live Support API endpoint not found',
