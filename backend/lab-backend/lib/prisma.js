/**
 * LAB BACKEND - DATABASE UTILITIES (PRISMA/POSTGRESQL)
...
const prisma = globalForPrisma.__lab_prisma ?? new PrismaClient({
  datasourceUrl: process.env.DATABASE_URL,
});
if (process.env.NODE_ENV !== 'production') globalForPrisma.__lab_prisma = prisma;

export { prisma };

async function connectDatabase() {
  await prisma.$connect();
  console.log('✅ Lab DB connected (PostgreSQL/Prisma)');
}

async function disconnectDatabase() {
  await prisma.$disconnect();
  console.log('🔌 Lab DB disconnected');
}
