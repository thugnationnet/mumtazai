/**
 * SOURCE UTILS — Shared helpers for canvas-app / canvas-build product separation.
 *
 ...
 *
 * Valid sources: 'canvas-app' | 'canvas-build'
 */

const VALID_SOURCES = ['canvas-app', 'canvas-build'];
...
export function requireSource(req, res, next) {
  ...
  if (!source) {
    return res.status(400).json({
      success: false,
      error: 'Missing or invalid source. Provide ?source=canvas-app or ?source=canvas-build',
    });
  }
  req.source = source;
  next();
 ...
 * Run: UPDATE canvas_projects SET source = 'canvas-app' WHERE source IS NULL;
