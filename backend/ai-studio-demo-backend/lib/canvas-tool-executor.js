...
export async function executeCanvasTool(toolName, input, context) {
  ...
  switch (toolName) {
    ...
    case 'canvas_agent_spawn': {
      const r = await agentSpawn(input.action, input, context.userId || 'default');
      return { result: r, files: context.files || [] };
    }
    ...
    default: {
      ...
      if (DELEGATED_AGENT_TOOLS.has(toolName)) {
        try {
          const agentResult = await executeToolCall(toolName, input, userId);
          return { result: agentResult, files };
        } catch (err) {
          return { result: { success: false, error: err.message }, files };
        }
      }
      return {
