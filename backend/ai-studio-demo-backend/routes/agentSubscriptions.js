  ...
  try {
      ...
      try {
        ...
        if (priceId) {
          ...
          const session = await stripeInstance.checkout.sessions.create({
            ...
            metadata: { userId, agentId, agentName: agentId, plan },
        ...
        console.error('[Stripe] No priceId configured for:', { agentId, plan, envPrefix });
    ...
    if (userRecord?.email) {
      sendPlanPurchaseEmail(userRecord.email, userRecord.name, {
        ...
        agentName: agentId === 'gencraft-pro' ? 'GenCraft Pro' : agentId,
        price: prices[plan],
        currency: 'USD',
        expiryDate,
