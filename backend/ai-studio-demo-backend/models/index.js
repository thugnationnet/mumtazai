  ...
  async save() {
    if (this._isNew) {
      ...
      const settings = {
        ...
        maxTokens: this.maxTokens || this.settings?.maxTokens || 2000,
...
class AgentMemoryAdapter {
  constructor(data = {}) {
    // Declare all instance properties with JSDoc types BEFORE Object.assign
    // so TypeScript can statically infer them on the class instance.
    /** @type {Array<{ type: string; content: string; data: Record<string, unknown>; importance: number; tags: string[]; createdAt?: Date; source?: { conversationId?: string; timestamp: Date } }>} */
    this.memories = [];
    /** @type {{ totalMessages: number; totalInteractions: number; lastInteraction: Date | null }} */
    this.relationship = { totalMessages: 0, totalInteractions: 0, lastInteraction: null };
    /** @type {Record<string, unknown>} */
    this.userProfile = {};
    /** @type {{ totalLearnings: number; correctPredictions: number; corrections: number; adaptations: number }} */
    this.learningMetrics = { totalLearnings: 0, correctPredictions: 0, corrections: 0, adaptations: 0 };
    /** @type {string} */
    this.userSummary = '';
    /** @type {{ preferredTopics: string[]; avoidTopics: string[]; commonQuestions: string[]; feedbackPatterns: { likes: string[]; dislikes: string[] } } | undefined} */
    this.conversationPatterns = undefined;

    // Overwrite defaults with actual DB data
    Object.assign(this, data);
    this._isNew = true;
  }
  ...
  _getData() {
    ...
    const schemaData = {};
    if (this.userId !== undefined) schemaData.userId = this.userId;
    if (this.agentId !== undefined) schemaData.agentId = this.agentId;
    if (this.memories !== undefined) schemaData.memories = this.memories;
    if (this.summary !== undefined) schemaData.summary = this.summary;
    if (this.totalMemories !== undefined) schemaData.totalMemories = this.totalMemories;
    if (this.lastAccessed !== undefined) schemaData.lastAccessed = this.lastAccessed;
    return schemaData;
  }

  /** Add a memory entry to the in-memory array */
  async addMemory(memoryData) {
    if (!Array.isArray(this.memories)) this.memories = [];
    this.memories.push({ ...memoryData, createdAt: new Date() });
  ...
  /** Record an interaction (updates relationship stats) */
  async recordInteraction() {
    if (!this.relationship) {
      this.relationship = { totalMessages: 0, totalInteractions: 0, lastInteraction: null };
    }
    this.relationship.totalInteractions = (this.relationship.totalInteractions || 0) + 1;
    this.relationship.lastInteraction = new Date();
    this.lastAccessed = new Date();
  }

  /** Build a context string from the most recent memories */
  buildContextString(limit = 10) {
    if (!Array.isArray(this.memories) || this.memories.length === 0) return '';
    const recent = this.memories.slice(-limit);
    return recent.map(m => `- ${m.content}`).join('\n');
  }

  /** Merge profile updates into the runtime userProfile object */
  ...
  /** Find an existing memory record or create a new one */
  static async getOrCreate(userId, agentId) {
    let instance = await AgentMemoryAdapter.findOne({ userId, agentId });
    if (!instance) {
      instance = new AgentMemoryAdapter({ userId, agentId, memories: [], totalMemories: 0 });
      await instance.save();
    }
    return instance;
  }
