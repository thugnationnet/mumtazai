/**
 * COMMUNITY - STANDALONE BACKEND SERVER
 * PostgreSQL/Prisma Backend (port 3011)
 *
 * Serves: /api/community/*, /api/suggestions/*, /api/user/*
 */
...
app.use((err, _req, res, _next) => {
  console.error('[community-backend] Error:', err);
...
async function gracefulShutdown(signal) {
  console.log(`\n${signal} received. Shutting down community-backend...`);
...
process.on('uncaughtException', (err) => console.error('[community-backend] Uncaught:', err));
process.on('unhandledRejection', (reason) => console.error('[community-backend] Unhandled:', reason));
