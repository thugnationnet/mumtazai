...
const AGENT_EXTRA_TOOLS = {
  'mrs-boss': [
    ...
    'execute_code', 'parse_pdf', 'parse_docx', 'parse_csv', 'parse_json',
    'data_visualize', 'report_generate', 'kpi_dashboard', 'trend_analyze',
    'invoice_generate', 'expense_track', 'budget_plan', 'financial_report', 'tax_calculate', 'currency_convert',
    'project_create', 'task_manage', 'milestone_track', 'gantt_generate', 'sprint_plan', 'deadline_track',
    'email_draft', 'email_template', 'calendar_manage', 'meeting_schedule',
    'proposal_generate', 'contract_draft', 'contract_analyze',
...
const SUB_INTENT_GROUPS = [
  // Finance / accounting
  { match: /\b(invoice|budget|expense|tax|financ|currency|payment|revenue|profit|cost|p&l|balance sheet)\b/,
    tools: ['invoice_generate', 'expense_track', 'budget_plan', 'financial_report', 'tax_calculate', 'currency_convert', 'report_generate', 'data_export', 'parse_pdf', 'parse_csv'] },
  // Project management
  { match: /\b(project|task|milestone|sprint|gantt|deadline|kanban|agile|scrum|roadmap)\b/,
    tools: ['project_create', 'task_manage', 'milestone_track', 'gantt_generate', 'sprint_plan', 'deadline_track'] },
  // Email / calendar / meetings
  { match: /\b(email|newsletter|calendar|meeting|schedul|appointment|reminder)\b/,
    tools: ['email_draft', 'email_template', 'calendar_manage', 'meeting_schedule'] },
  // Legal / contracts
  { match: /\b(proposal|contract|nda|legal|agreement|terms|clause)\b/,
    tools: ['proposal_generate', 'contract_draft', 'contract_analyze', 'parse_pdf', 'parse_docx'] },
  // Reporting / dashboards / KPIs
  { match: /\b(chart|graph|kpi|dashboard|pivot|trend|forecast|visualiz|report|metric)\b/,
    tools: ['data_visualize', 'report_generate', 'kpi_dashboard', 'trend_analyze', 'pivot_table', 'data_export'] },
  // Document parsing
  { match: /\b(parse|pdf|docx|csv|extract.+(?:data|text)|read.+document|spreadsheet)\b/,
    tools: ['parse_pdf', 'parse_docx', 'parse_csv', 'parse_json', 'execute_code'] },
];
