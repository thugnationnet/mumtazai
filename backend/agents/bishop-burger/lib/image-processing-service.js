 ...
 *
 * Uses AI vision models (GPT-4o / Claude) for detection — no local models needed.
 *
 * Actions:
...
/**
 * Face detection — Azure Vision (primary) → GPT-4o / Claude / Gemini (fallback)
...
async function _faceDetectViaAI(buffer, action, options, userId) {
  ...
  // ── AI VISION FALLBACK (GPT-4o → Claude → Gemini) ─────────────
  const base64 = buffer.toString('base64');
  ...
  if (process.env.ANTHROPIC_API_KEY) {
    try {
      ...
      console.error('Anthropic face detection failed:', e.message);
    }
  }
  ...
  if (process.env.GEMINI_API_KEY) {
    try {
      ...
      if (jsonMatch) {
        ...
        return _handleAIFaceResult(parsed, buffer, action, options, userId);
      }
    } catch (e) {
      console.error('Gemini face detection failed:', e.message);
    }
  }
  ...
  return {
    success: false,
    error:
      'Face detection requires a vision AI provider. Set OPENAI_API_KEY, ANTHROPIC_API_KEY, or GEMINI_API_KEY.',
  };
}
...
/**
 * Call the best available vision model — tries OpenAI → Anthropic → Gemini
 */
async function _callVisionModel(base64, prompt, base64B = null, options = {}) {
  const preferredProvider = options.provider || options.model;

  // Build provider attempts in priority order
  const providers = [];
  if (preferredProvider === 'anthropic' || preferredProvider === 'claude') {
    providers.push('anthropic', 'openai', 'gemini');
  } else if (preferredProvider === 'gemini' || preferredProvider === 'google') {
    providers.push('gemini', 'openai', 'anthropic');
  } else {
    providers.push('openai', 'anthropic', 'gemini');
  }
    ...
    try {
      switch (provider) {
        ...
        case 'anthropic': {
          ...
          const response = await anthropic.messages.create({
            model: options.anthropic_model || 'claude-sonnet-4-20250514',
            max_tokens: options.max_tokens || 2000,
            messages: [{ role: 'user', content }],
          });
          return {
        ...
        case 'gemini': {
          ...
          const model = genAI.getGenerativeModel({
            model: options.gemini_model || 'gemini-2.0-flash',
          });
          const parts = [
            { text: prompt },
            { inlineData: { mimeType: 'image/png', data: base64 } },
  ...
  return {
    success: false,
    error:
      'No vision-capable AI provider available. Set OPENAI_API_KEY, ANTHROPIC_API_KEY, or GEMINI_API_KEY.',
  };
}
