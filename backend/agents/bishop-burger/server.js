/**
 * BISHOP-BURGER — Dedicated Isolated Agent Backend
 *
...
// CORS — bishop-burger accepts requests from its own subdomain + main domains
...
app.use('/api/agent/chat-stream', (req, res, next) => {
  ...
  if (entry.count >= RATE_LIMIT_MAX) {
    return res.status(429).json({
      error: `Rate limit exceeded. bishop-burger allows ${RATE_LIMIT_MAX} requests/hour per IP.`,
    });
  }
  entry.count++;
  next();
...
// Lock agentId — this backend only serves bishop-burger
app.use('/api/agent/chat-stream', (req, res, next) => {
  if (req.method === 'POST') {
    req.body = req.body || {};
...
app.use((req, res) => {
  res.status(404).json({
    error: `bishop-burger backend — only /api/agent/chat-stream is served here`,
  });
});
