/**
 * COMEDY-KING — Dedicated Isolated Agent Backend
 *
...
// CORS — comedy-king accepts requests from its own subdomain + main domains
...
app.use('/api/agent/chat-stream', (req, res, next) => {
  ...
  if (entry.count >= RATE_LIMIT_MAX) {
    return res.status(429).json({
      error: `Rate limit exceeded. comedy-king allows ${RATE_LIMIT_MAX} requests/hour per IP.`,
    });
  }
  entry.count++;
  next();
...
// Lock agentId — this backend only serves comedy-king
app.use('/api/agent/chat-stream', (req, res, next) => {
  if (req.method === 'POST') {
    req.body = req.body || {};
...
app.use((req, res) => {
  res.status(404).json({
    error: `comedy-king backend — only /api/agent/chat-stream is served here`,
  });
});
...
// Graceful shutdown
process.on('SIGTERM', () => {
  console.log(`[agent-${AGENT_ID}] SIGTERM received, shutting down...`);
  process.exit(0);
});
