...
const AGENT_EXTRA_TOOLS = {
  'einstein': [
    'execute_code', 'generate_code', 'debug_code', 'http_request',
    'data_visualize', 'report_generate', 'trend_analyze', 'forecast_model',
    'data_profile', 'data_clean', 'parse_json', 'parse_csv', 'parse_pdf',
    'summarize_file', 'search_in_files', 'read_json',
  ],
};
...
/**
 * Try to narrow `baseTools` by the first matching SUB_INTENT_GROUP.
 * Returns the narrowed tool array, or null if no group matched / not a heavy agent.
 */
...
// Cache the agent's full (un-narrowed) tool array so it has a stable reference
// — required for the WeakMap caches in provider-tool-router and agent-tools-service.
 ...
 * @param {Array} allTools - TOOL_DEFINITIONS
 * @param {string} [userMessage] - latest user message (for sub-intent)
 * @returns {Array} tool definitions to send to the LLM
 */
export function getFastToolsForAgent(agentId, allTools, userMessage) {
  // Stable per-agent base array (cache by agentId — input ref doesn't change at runtime)
  ...
  // Heavy agents may sub-narrow per request (dynamic — no cache, but small set)
