/**
 * UNIVERSAL CHAT BACKEND - DATABASE UTILITIES (PRISMA/POSTGRESQL)
 * Standalone PrismaClient for universal_chat_db
 */

import { PrismaClient } from '@prisma/client';

const globalForPrisma = globalThis;
const prisma = globalForPrisma.__universal_chat_prisma ?? new PrismaClient({
  datasourceUrl: process.env.UNIVERSAL_CHAT_DATABASE_URL || process.env.DATABASE_URL,
});
if (process.env.NODE_ENV !== 'production') globalForPrisma.__universal_chat_prisma = prisma;

export { prisma };

async function connectDatabase() {
  await prisma.$connect();
  console.log('✅ Universal Chat DB connected (PostgreSQL/Prisma)');
}

async function disconnectDatabase() {
  await prisma.$disconnect();
  console.log('🔌 Universal Chat DB disconnected');
}
