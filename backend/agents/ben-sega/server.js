/**
 * BEN-SEGA — Dedicated Isolated Agent Backend
 *
 * This agent has its own:
 *   - Port 4002
 *   - .env (in this directory)
 *   - API keys (Mistral PRIMARY + RunwayML for video)
 *   - Rate limits
 *   - Provider cascade
 *
...
// CORS — ben-sega accepts requests from its own subdomain + main domains
...
app.use('/api/agent/chat-stream', (req, res, next) => {
  ...
  if (entry.count >= RATE_LIMIT_MAX) {
    return res.status(429).json({
      error: `Rate limit exceeded. ben-sega allows ${RATE_LIMIT_MAX} requests/hour per IP.`,
    });
  }
  entry.count++;
  next();
...
// Lock agentId — this backend only serves ben-sega
app.use('/api/agent/chat-stream', (req, res, next) => {
  if (req.method === 'POST') {
    req.body = req.body || {};
...
// Mount the shared chat-stream route handler
...
app.use((req, res) => {
  res.status(404).json({
    error: `ben-sega backend — only /api/agent/chat-stream is served here`,
  });
});

// Start
app.listen(PORT, () => {
  console.log(`[agent-${AGENT_ID}] ✅ Dedicated backend running on port ${PORT}`);
});

// Graceful shutdown
process.on('SIGTERM', () => {
  console.log(`[agent-${AGENT_ID}] SIGTERM received — shutting down gracefully`);
  process.exit(0);
});
process.on('SIGINT', () => {
  console.log(`[agent-${AGENT_ID}] SIGINT received — shutting down`);
  process.exit(0);
});
