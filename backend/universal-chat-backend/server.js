/**
 * UNIVERSAL CHAT - STANDALONE BACKEND SERVER
 * PostgreSQL/Prisma Backend (port 3008)
 */
...
// ── Inline editor state (replaces agent-intelligence-service) ──
let _io = null;
function setSocketIO(io) { _io = io; }
const editorStates = new Map();
function updateEditorState(userId, state) {
...
async function checkAIProviders() {
  const providerConfigs = [
    { key: 'MISTRAL_API_KEY', name: 'Mistral (Large)', model: 'mistral-large-latest', url: 'https://api.mistral.ai/v1/models', authHeader: 'Bearer' },
    { key: 'XAI_API_KEY', name: 'xAI (Grok)', model: 'grok-3-mini-fast', url: 'https://api.x.ai/v1/models', authHeader: 'Bearer' },
...
async function initializeServer() {
  try {
    console.log('🔧 Initializing Universal Chat Backend...');
    console.log('📦 Database: PostgreSQL via Prisma (universal_chat_db)');
    ...
    server.listen(PORT, host, () => {
      console.log(`🚀 Universal Chat Backend running on ${host}:${PORT}`);
