...
// Default preferences returned for unauthenticated users
...
// Uses optionalAuth: authenticated users get stored prefs, guests get defaults
router.get('/', optionalAuth, async (req, res) => {
  try {
    // Unauthenticated users get sensible defaults (no DB lookup needed)
    if (!req.isAuthenticated || !req.userId || String(req.userId).startsWith('guest_') || String(req.userId).startsWith('anon_')) {
      return res.json({ success: true, data: DEFAULT_PREFERENCES });
    }
...
router.put('/agent-settings/:agentId', requireAuth, async (req, res) => {
  try {
...
router.put('/agent-theme/:agentId', requireAuth, async (req, res) => {
  try {
    const { agentId } = req.params;
...
router.put('/site-theme', requireAuth, async (req, res) => {
  try {
...
router.put('/agent-preferences', requireAuth, async (req, res) => {
  try {
...
router.put('/voice-settings', requireAuth, async (req, res) => {
  try {
...
router.get('/canvas-messages/:agentId', requireAuth, async (req, res) => {
  try {
    const { agentId } = req.params;
    const prefs = await prisma.userPreferences.findUnique({
      where: { userId: req.userId },
...
router.put('/canvas-messages/:agentId', requireAuth, async (req, res) => {
  try {
    const { agentId } = req.params;
...
router.put('/ui-flags', requireAuth, async (req, res) => {
  try {
...
router.get('/tool-state/:toolName', requireAuth, async (req, res) => {
  try {
...
router.put('/tool-state/:toolName', requireAuth, async (req, res) => {
  try {
    const { toolName } = req.params;
...
router.put('/message-feedback', requireAuth, async (req, res) => {
  try {
...
router.get('/canvas-state', requireAuth, async (req, res) => {
  try {
    const prefs = await prisma.userPreferences.findUnique({
      where: { userId: req.userId },
    });
...
router.get('/canvas-state/:key', requireAuth, async (req, res) => {
  try {
...
router.put('/canvas-state/:key', requireAuth, async (req, res) => {
  try {
    const { key } = req.params;
...
router.delete('/canvas-state/:key', requireAuth, async (req, res) => {
  try {
    const { key } = req.params;
    if (!key || typeof key !== 'string') {
      return res.status(400).json({ success: false, message: 'key is required' });
...
router.get('/tool-state/:toolId', requireAuth, async (req, res) => {
  try {
...
router.put('/tool-state/:toolId', requireAuth, async (req, res) => {
  try {
    const { toolId } = req.params;
    if (!toolId || typeof toolId !== 'string') {
      return res.status(400).json({ success: false, message: 'toolId is required' });
