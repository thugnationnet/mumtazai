...
/**
 * Send a tracked email — creates a tracking record, embeds pixel, sends email, updates delivery status
...
async function sendTrackedEmail(to, subject, emailType, htmlBuilder, userId, metadata) {
  ...
  // Update delivery status in tracking record
  if (recordId) {
    try {
      if (result.success) {
        await prisma.emailTracking.update({
          ...
          data: {
            ...
            metadata: metadata ? { ...metadata, messageId: result.messageId } : { messageId: result.messageId },
          },
        });
      } else {
        await prisma.emailTracking.update({
      ...
      console.error('[EMAIL] Tracking status update failed:', err.message);
    }
  }
...
async function sendAdminEmail(subject, html, text, emailType = 'admin_notification') {
  return sendTrackedEmail(ADMIN_EMAIL, subject, emailType, () => html, null, { isAdminEmail: true });
}
...
export async function notifyAdminJobApplication(data) {
  ...
  return sendAdminEmail(subject, html, `Application: ${data.position}\nFrom: ${data.applicantName} (${data.applicantEmail})`, 'admin_application');
}
...
export async function notifyAdminNewUser(data) {
  ...
  return sendAdminEmail(subject, html, `New User: ${data.name} (${data.email})`, 'admin_new_user');
}
