...
async function canvasBuildProcessor(job) {
  ...
  // Fetch project & build record (files is a Json column, not a relation — no include needed)
...
async function canvasDeployProcessor(job) {
  ...
  const deployUrl = project.deploymentUrl || `https://${project.name}.${process.env.DEPLOY_HOST || 'apps.mumtaz.ai'}`;
  await prisma.canvasProject.update({
    where: { id: projectId },
    data: {
