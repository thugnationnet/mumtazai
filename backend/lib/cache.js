...
const MAX_MEMORY_CACHE_SIZE = 10000; // Prevent unbounded memory growth
    ...
    try {
      ...
      if (this.client && this.isConnected) {
        ...
        if (this.memoryCache.size >= MAX_MEMORY_CACHE_SIZE) {
          const keysToDelete = [...this.memoryCache.keys()].slice(0, Math.floor(MAX_MEMORY_CACHE_SIZE * 0.2));
    ...
    return async (req, res, next) => {
      ...
      try {
        ...
        res.json = function (data) {
          cacheInstance.get(key).then((cached) => {
            if (!cached) {
