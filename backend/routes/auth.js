...
// JWT secret for neural_link_session cookie — MUST be set in environment
...
// Feature flag: skip the email-OTP step on login and the verification email on signup.
// Defaults to true; set SKIP_EMAIL_VERIFICATION=false to re-enable the OTP flow.
const SKIP_EMAIL_VERIFICATION = (process.env.SKIP_EMAIL_VERIFICATION ?? 'true').toLowerCase() !== 'false';

// Strict rate limiter for 2FA/OTP verification (brute-force protection)
  ...
  try {
    ...
    if (!user) {
      ...
      return res.status(lockResult.locked ? 423 : 401).json({
        success: false,
        message: lockResult.locked ? lockResult.message : 'Invalid email or password',
        ...(lockResult.locked ? { locked: true, tier: lockResult.tier, retryAfter: lockResult.retryAfter } : {}),
      });
    }
    ...
    // No 2FA — email OTP step (gated by SKIP_EMAIL_VERIFICATION flag)
    if (SKIP_EMAIL_VERIFICATION) {
      // Bypass OTP: create session immediately
...
router.post('/signup', async (req, res) => {
  try {
    const { email, name, password, authMethod, turnstileToken } = req.body;
    ...
    // Send verification code + welcome email (verification skipped when flag is on)
    if (!SKIP_EMAIL_VERIFICATION) {
...
router.get('/session', async (req, res) => {
  try {
    ...
    // Refresh cookies on every valid session check to heal legacy host-only cookies
    // and maintain cross-subdomain auth consistency.
    setSessionCookies(res, req, sessionId, { userId: user.id, email: user.email });
...
const handleAuthVerify = async (req, res) => {
  try {
    ...
    // Same cookie refresh behavior as /session for clients using /verify alias.
    setSessionCookies(res, req, sessionId, { userId: user.id, email: user.email });
...
router.get('/google', (req, res) => {
  ...
  // Preserve ?redirect= param through OAuth flow — validate to prevent open redirect
  const rawRedirect = req.query.redirect || '/dashboard/overview';
  // Allow relative paths or absolute URLs to *.mumtaz.ai subdomains
  const isRelativePath = typeof rawRedirect === 'string' && rawRedirect.startsWith('/') && !rawRedirect.startsWith('//');
  const isOnelastaiDomain = typeof rawRedirect === 'string' && /^https:\/\/([a-z0-9-]+\.)*mumtazai\.co(\/.*)?$/i.test(rawRedirect);
  const redirect = (isRelativePath || isOnelastaiDomain) ? rawRedirect : '/dashboard/overview';
...
router.get('/github', (req, res) => {
  ...
  const rawRedirectGh = req.query.redirect || '/dashboard/overview';
  const isRelativePathGh = typeof rawRedirectGh === 'string' && rawRedirectGh.startsWith('/') && !rawRedirectGh.startsWith('//');
  const isOnelastaiDomainGh = typeof rawRedirectGh === 'string' && /^https:\/\/([a-z0-9-]+\.)*mumtazai\.co(\/.*)?$/i.test(rawRedirectGh);
  const redirectGh = (isRelativePathGh || isOnelastaiDomainGh) ? rawRedirectGh : '/dashboard/overview';
  res.cookie('oauth_redirect', redirectGh, {
    httpOnly: true,
    secure: isProduction,
    sameSite: 'lax',
    maxAge: 10 * 60 * 1000,
...
router.get('/yahoo', (req, res) => {
  ...
  const rawRedirectYh = req.query.redirect || '/dashboard/overview';
  const isRelativePathYh = typeof rawRedirectYh === 'string' && rawRedirectYh.startsWith('/') && !rawRedirectYh.startsWith('//');
  const isOnelastaiDomainYh = typeof rawRedirectYh === 'string' && /^https:\/\/([a-z0-9-]+\.)*mumtazai\.co(\/.*)?$/i.test(rawRedirectYh);
  const redirectYh = (isRelativePathYh || isOnelastaiDomainYh) ? rawRedirectYh : '/dashboard/overview';
  res.cookie('oauth_redirect', redirectYh, {
    httpOnly: true,
    secure: isProduction,
    sameSite: 'lax',
    maxAge: 10 * 60 * 1000,
...
router.get('/microsoft', (req, res) => {
  ...
  const rawRedirectMs = req.query.redirect || '/dashboard/overview';
  const isRelativePathMs = typeof rawRedirectMs === 'string' && rawRedirectMs.startsWith('/') && !rawRedirectMs.startsWith('//');
  const isOnelastaiDomainMs = typeof rawRedirectMs === 'string' && /^https:\/\/([a-z0-9-]+\.)*mumtazai\.co(\/.*)?$/i.test(rawRedirectMs);
  const redirectMs = (isRelativePathMs || isOnelastaiDomainMs) ? rawRedirectMs : '/dashboard/overview';
  res.cookie('oauth_redirect', redirectMs, {
    httpOnly: true,
    secure: isProduction,
    sameSite: 'lax',
    maxAge: 10 * 60 * 1000,
...
router.post('/reset-password', async (req, res) => {
  try {
    const { token, email, password, turnstileToken } = req.body;
...
// =====================================================
// DELETE ACCOUNT (Google Play Store requirement)
// =====================================================
...
router.delete('/account', async (req, res) => {
  try {
    ...
    // Verify password confirmation (require password for security)
    ...
    if (user.authMethod === 'password' && user.password) {
      if (!password) {
        return res.status(400).json({ success: false, message: 'Password is required' });
      }
    ...
    // (PushSubscription, UserSecurity, AgentSubscription, AgentPersonalization,
    //  AgentMemory, ChatSession, ChatFeedback, ChatSettings, Transaction,
    //  UserFavorites, CanvasProject, CanvasBuildProject, CanvasDeployCredential,
    //  CanvasAsset, AppHostingPlan, AppUsageRecord, UserMemoryProfile, UserPreferences)
    await prisma.user.delete({ where: { id: userId } });

    console.log(`[Account Deletion] User ${userId} (${userEmail}) deleted successfully`);
...
// GET endpoint for account deletion page (Google Play data safety URL)
router.get('/account/delete-info', (req, res) => {
  res.json({
    success: true,
    message: 'Account deletion is available',
    instructions: 'To delete your account, log in and visit /account/delete, or send a DELETE request to /api/auth/account with your session cookie and confirmation.',
    deletionScope: [
      'User profile and credentials',
      'Chat sessions and messages',
      'Agent subscriptions and personalizations',
      'Canvas projects and assets',
      'Transaction history',
      'Favorites and preferences',
      'Push notification subscriptions',
    ],
    timeframe: 'Immediate — all data is deleted upon confirmation',
    contact: 'support@mumtaz.ai',
  });
});
