  ...
  return (
    <div className="min-h-full themed-section-bg">
        ...
        <div className="max-w-[1600px] mx-auto px-6 relative z-10">
          <div className="bg-white/30 backdrop-blur-2xl border border-white/50 rounded-3xl p-6 md:p-8 shadow-[0_8px_40px_rgba(139,92,246,0.12),inset_0_1px_0_rgba(255,255,255,0.6)]">
            <div className="flex items-center justify-between">
              <div>
                <div className="inline-flex items-center gap-2 px-3 py-1 bg-teal-500/10 border border-teal-300/40 rounded-full text-teal-700 text-xs font-bold uppercase tracking-wider mb-3">
                  <span className="w-2 h-2 bg-teal-500 rounded-full animate-pulse" />
                  Visitors
                </div>
                <h1 className="text-2xl md:text-3xl font-black bg-gradient-to-r from-slate-800 via-purple-800 to-teal-700 bg-clip-text text-transparent">Visitors & Sessions</h1>
                <p className="text-sm text-slate-500 mt-1">Track every visitor, device, IP, and session in real-time</p>
              </div>
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => tab === 'visitors' ? fetchVisitors() : fetchSessions()} disabled={loading} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-white/50 text-slate-500 hover:text-slate-700 border border-white/60 hover:border-purple-300 hover:shadow transition-all">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
            tab === 'visitors'
              ? 'bg-white/60 text-purple-600 border border-purple-200 shadow-sm'
              : 'text-slate-500 hover:text-slate-700 bg-white/50 border border-white/60'
          }`}
        >
      ...
      {tab === 'visitors' && visitors.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="relative glass-card p-5">
            <p className="text-[10px] text-slate-400 uppercase tracking-wider">Total (this page)</p>
            <p className="text-2xl font-bold text-slate-700 mt-1">{visitors.length}</p>
            <p className="text-[10px] text-slate-400 mt-1">{pagination.total.toLocaleString()} total across all pages</p>
          </div>
          <div className="relative glass-card p-5">
            <p className="text-[10px] text-slate-400 uppercase tracking-wider">Active Now</p>
            <p className="text-2xl font-bold text-green-600 mt-1">{activeCount}</p>
            <p className="text-[10px] text-slate-400 mt-1">{goneCount} gone</p>
          </div>
          <div className="relative glass-card p-5">
            <p className="text-[10px] text-slate-400 uppercase tracking-wider">Registered</p>
            <p className="text-2xl font-bold text-blue-600 mt-1">{registeredCount}</p>
            <p className="text-[10px] text-slate-400 mt-1">{visitors.length - registeredCount} anonymous</p>
          </div>
          <div className="relative glass-card p-5">
                  ...
                  visitors.map((v) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <DevIcon className="w-4 h-4 text-slate-400 inline" />
                          <span className="ml-1.5 text-slate-500 capitalize">
                            {v.device}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3 text-slate-500">
                          <Globe className="w-3 h-3 inline mr-1 text-slate-400" />
                          {v.city}, {v.country}
                        </td>
                        <td className="px-4 py-3 text-slate-500 font-mono truncate max-w-[120px]">
                          {v.landingPage}
                        </td>
                ...
                {loading && sessions.length === 0 ? (
                  ...
                  sessions.map((s) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-purple-600 text-center">
                        {s.events}
                      </td>
                      <td className="px-4 py-3">
                        {s.isActive ? (
                          <span className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-green-50 text-green-600 text-[10px] font-medium w-fit">
      ...
      {selectedVisitor && visitorDetail && (
        ...
        >
          <div
            className="bg-white/80 backdrop-blur-2xl border border-white/80 rounded-3xl shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-y-auto"
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div key={label} className="bg-white/40 rounded-xl p-3">
                  ...
                  <div className="space-y-1.5 max-h-40 overflow-y-auto">
                    {visitorDetail.sessions.map((s: any) => (
                      <div
                        ...
                        className="flex items-center gap-3 px-3 py-2 bg-white/40 rounded-lg text-xs"
                      >
                        <span
                          className={`w-2 h-2 rounded-full ${s.isActive ? 'bg-green-500' : 'bg-slate-300'}`}
                        />
                  ...
                  <div className="space-y-1 max-h-40 overflow-y-auto">
                    {visitorDetail.pageViews
                      ...
                      .map((pv: any, i: number) => (
                        <div
                          key={i}
                          className="flex items-center gap-3 px-3 py-1.5 text-xs hover:bg-slate-50 rounded"
                        >
