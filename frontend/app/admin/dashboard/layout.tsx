  ...
  useEffect(() => {
    ...
    async function checkAdmin() {
      try {
        ...
        // Check admin access via email allow-list (role field may not be set)
        const userEmail = (data.user.email || '').toLowerCase();
        if (data.user.role !== 'admin' && !ADMIN_EMAILS.includes(userEmail)) {
          router.replace('/');
          return;
        }
    ...
    return (
      <div className="min-h-screen bg-[#060B18] flex items-center justify-center p-4">
        <div className="w-full max-w-md">
          <div className="bg-[#0F1A2E] border border-sky-500/15 rounded-2xl p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">{gateUserEmail}</p>
              </div>
            </div>
            ...
            <form onSubmit={handleGateSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 bg-[#060B18] border border-sky-500/15 rounded-lg text-white text-sm placeholder-slate-500 focus:outline-none focus:border-sky-500/50 focus:ring-1 focus:ring-sky-500/20"
              ...
              {gateNeeds2FA && (
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2.5 bg-[#060B18] border border-sky-500/15 rounded-lg text-white text-sm placeholder-slate-500 focus:outline-none focus:border-sky-500/50 focus:ring-1 focus:ring-sky-500/20 font-mono tracking-widest text-center"
                    placeholder="000000"
                    maxLength={6}
                  />
                </div>
              ...
              <button
                ...
                className="w-full py-2.5 bg-gradient-to-r from-sky-600 to-cyan-500 hover:from-sky-400 hover:to-cyan-400 text-white text-sm font-medium rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
            ...
            <button
              ...
              className="w-full mt-3 py-2 text-xs text-slate-400 hover:text-slate-300 transition-colors"
            >
              ← Back to Site
            </button>
          </div>
    ...
    <div className="min-h-screen bg-[#060B18] text-white flex">
      ...
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 flex flex-col
          bg-[#0A1128] border-r border-sky-500/10 transition-all duration-300
          ${collapsed ? 'w-[68px]' : 'w-[260px]'}
          ${mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
        `}
      >
        {/* Logo */}
        <div className="flex items-center gap-3 px-4 h-16 border-b border-sky-500/10 shrink-0">
          <div className="w-8 h-8 neu-icon text-sm font-bold shrink-0">
            OL
          </div>
          {!collapsed && (
            <div className="overflow-hidden">
              ...
              <p className="text-[10px] text-slate-400">Mumtaz AI</p>
            </div>
          )}
        </div>
              ...
              <Link
                ...
                className={`group flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all ${
                  isActive
                    ...
                    : 'text-slate-400 hover:text-white hover:bg-sky-500/[0.08] border border-transparent'
                }`}
        ...
        {/* Collapse toggle */}
        <div className="border-t border-sky-500/10 p-2 hidden lg:block">
          <button
            onClick={() => setCollapsed(!collapsed)}
            className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs text-slate-400 hover:text-slate-300 hover:bg-sky-500/[0.08] transition-colors"
          >
            {collapsed ? (
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top bar */}
        <header className="h-14 border-b border-sky-500/10 bg-[#0A1128]/80 backdrop-blur-sm flex items-center justify-between px-4 shrink-0 sticky top-0 z-30">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setMobileOpen(true)}
              className="lg:hidden p-1.5 rounded-lg hover:bg-sky-500/10 text-slate-400"
            >
          ...
          <div className="flex items-center gap-2">
            <Link
              href="/"
              className="text-xs text-slate-400 hover:text-slate-300 px-3 py-1.5 rounded-lg hover:bg-sky-500/[0.08] transition-colors"
            >
              ← Back to Site
            </Link>
