      ...
      <div className="bg-gradient-to-r from-brand-600 to-accent-600 py-12">
        <div className="container-custom">
          ...
          <div className="text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-bold mb-4 text-white">QR Code <span className="text-blue-100">Generator</span></h1>
            <p className="text-xl text-blue-100 max-w-2xl mx-auto">Generate QR codes for URLs, Wi-Fi, contacts, events, and more — with full customization</p>
          </div>
        </div>
      </div>
          ...
          <div className="lg:col-span-2 space-y-4">
                ...
                {templates.map(t => (
                  <button
                    ...
                    className={`flex flex-col items-center gap-1 p-3 rounded-xl text-xs font-medium transition-all
                      ${contentType === t.id ? 'ring-2 ring-brand-500 bg-brand-50' : 'hover:bg-gray-50 bg-white border border-gray-100'}`}
                  >
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6 space-y-5">
              ...
              <div>
                ...
                <input type="range" min={128} max={1024} step={8} value={size} onChange={(e) => setSize(parseInt(e.target.value))} className="w-full accent-brand-600" />
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {presetSizes.map(s => (
                    <button key={s} onClick={() => setSize(s)} className={`px-2 py-1 rounded text-xs transition-colors ${size === s ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                      {s}px
                    </button>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-4 gap-2 mt-2">
                  {ecLevels.map(ec => (
                    <button
                      ...
                      className={`p-2 rounded-lg text-center transition-all ${ecLevel === ec.value ? 'ring-2 ring-brand-500 bg-brand-50' : 'bg-gray-50 hover:bg-gray-100 border border-gray-200'}`}
                    >
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {presetColors.map((c, i) => (
                    <button
                      ...
                      className={`w-10 h-10 rounded-lg border-2 transition-all overflow-hidden ${fgColor === c.fg && bgColor === c.bg ? 'border-brand-500 scale-110' : 'border-gray-200'}`}
                      title={c.label}
                    >
              ...
              <div>
                ...
                <input type="range" min={0} max={10} value={margin} onChange={(e) => setMargin(parseInt(e.target.value))} className="w-full accent-brand-600" />
              </div>
            </div>
          </div>
              ...
              <div className="space-y-2">
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className="px-3 py-2.5 bg-brand-600 hover:bg-brand-700 text-white rounded-lg transition-colors flex items-center justify-center gap-1.5 text-sm font-medium disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    disabled={!isContentReady}
                    className="px-3 py-2.5 bg-accent-600 hover:bg-accent-700 text-white rounded-lg transition-colors flex items-center justify-center gap-1.5 text-sm font-medium disabled:opacity-50"
                  >
