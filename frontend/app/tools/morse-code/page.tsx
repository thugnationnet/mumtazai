      ...
      <div className="relative py-14 md:py-20 overflow-hidden rounded-b-[2rem] themed-section-bg">
        ...
        <div className="container-custom relative z-10">
          ...
          <div className="text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-bold mb-4 font-black bg-gradient-to-r from-slate-800 via-purple-800 to-indigo-700 bg-clip-text text-transparent">Morse Code <span className="text-slate-500">Translator</span></h1>
            <p className="text-xl text-slate-500 max-w-2xl mx-auto">Encode, decode & play Morse code with adjustable speed and frequency</p>
          </div>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="bg-white/40 backdrop-blur-lg rounded-2xl border border-white/60 shadow-lg p-6">
            <div className="flex flex-wrap gap-3 items-center">
              <div className="flex gap-2">
                <button onClick={() => setMode('encode')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${mode === 'encode' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                  Text → Morse
                </button>
                ...
                <button onClick={() => setMode('decode')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${mode === 'decode' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                  Morse → Text
                </button>
              </div>
          ...
          {output && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              {/* Play Controls */}
              <div className="bg-white/40 backdrop-blur-lg rounded-2xl border border-white/60 shadow-lg p-6 lg:col-span-2">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <button onClick={() => setShowSettings(!showSettings)} className={`p-3 rounded-xl transition-colors ${showSettings ? 'bg-blue-50 text-blue-600 border border-blue-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                ...
                {showSettings && (
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-gray-50 rounded-xl p-4">
                    <div>
                      ...
                      <input type="range" min={5} max={40} value={wpm} onChange={(e) => setWpm(parseInt(e.target.value))} className="w-full accent-blue-600" />
                    </div>
                    <div>
                      <div className="flex justify-between mb-1">
                      ...
                      <input type="range" min={300} max={1200} step={50} value={frequency} onChange={(e) => setFrequency(parseInt(e.target.value))} className="w-full accent-blue-600" />
                    </div>
                    <div>
                      <div className="flex justify-between mb-1">
                      ...
                      <input type="range" min={5} max={100} value={Math.round(volume * 100)} onChange={(e) => setVolume(parseInt(e.target.value) / 100)} className="w-full accent-blue-600" />
                    </div>
                  </div>
                )}
          ...
          <div className="bg-white/40 backdrop-blur-lg rounded-2xl border border-white/60 shadow-lg p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2">
              {Object.entries(PROSIGNS).map(([name, { code, meaning }]) => (
                <div key={name} className="bg-gray-50 rounded-xl p-3 border border-gray-200 hover:border-blue-300 transition-colors">
                  <div className="font-bold text-gray-900 text-sm">{name}</div>
                  <div className="font-mono text-xs text-blue-600">{code}</div>
              ...
              <div className="mt-4 grid grid-cols-4 sm:grid-cols-6 md:grid-cols-9 lg:grid-cols-13 gap-2">
                {referenceChars.slice(0, 36).map(([char, code]) => (
                  <button
                    ...
                    className={`rounded-lg p-2 text-center border transition-all cursor-pointer ${activeElement === char ? 'bg-blue-50 border-blue-300 scale-105' : 'bg-gray-50 border-gray-200 hover:bg-gray-100'}`}
                  >
                    ...
                    <div className="flex items-center justify-center gap-[2px] mt-1 h-2">
                      {code.split('').map((c, i) => c === '.' ? <div key={i} className="w-1 h-1 bg-blue-500 rounded-full" /> : <div key={i} className="w-3 h-1 bg-blue-500 rounded-full" />)}
                    </div>
                  </button>
                ))}
              </div>
