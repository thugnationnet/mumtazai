      ...
      <div className="relative py-14 md:py-20 overflow-hidden rounded-b-[2rem] themed-section-bg">
        ...
        <div className="container-custom relative z-10">
          ...
          <div className="text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-bold mb-4 font-black bg-gradient-to-r from-slate-800 via-purple-800 to-indigo-700 bg-clip-text text-transparent">IPv4 to <span className="text-slate-500">IPv6 Converter</span></h1>
            <p className="text-xl text-slate-500 max-w-2xl mx-auto">Convert IPv4 addresses to all IPv6 notations with batch support</p>
          </div>
          </div>
        </div>
      </div>
      ...
      <main className="container-custom py-8">
        <div className="max-w-3xl mx-auto space-y-6">
            ...
            {mode === 'single' ? (
              <>
                <div className="flex gap-3">
                  ...
                  <button onClick={convert} className="px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg font-semibold hover:opacity-90 transition-opacity flex items-center gap-2">
              ...
              <>
                ...
                <button onClick={convertBatch} className="w-full mt-3 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg font-semibold hover:opacity-90 transition-opacity">Convert All</button>
              </>
            )}
          ...
          {mode === 'single' && result && (
            <div className="bg-white/40 backdrop-blur-lg rounded-2xl border border-white/60 shadow-lg p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">All Representations of <span className="font-mono text-blue-600">{result.ipv4}</span></h3>
                <button onClick={() => { const all = formats(result).map(f => `${f.label}: ${f.value}`).join('\n'); copy(all, 'all') }} className="text-xs text-blue-600 hover:text-blue-700 flex items-center gap-1">
                    ...
                    {'error' in r ? (
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 mt-2">
                        {[
                          ...
                          <button key={f.l} onClick={() => copy(f.v, `${i}-${f.l}`)} className="text-left bg-white rounded px-2 py-1 border border-gray-200 hover:border-blue-300 transition-colors group">
