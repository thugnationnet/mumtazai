      ...
      <div className="bg-gradient-to-r from-brand-600 to-accent-600 py-12">
        <div className="container-custom">
          ...
          <div className="text-center">
            <div className="flex items-center justify-center mb-6"><div className="w-20 h-20 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center shadow-lg"><Key className="w-10 h-10 text-white" /></div></div>
            <h1 className="text-4xl md:text-5xl font-bold mb-4 text-white">DNSKEY / DS <span className="text-blue-100">Lookup</span></h1>
            <p className="text-xl text-blue-100 max-w-2xl mx-auto">Check DNSSEC key and delegation signer records</p>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6">
            <div className="flex gap-3">
              <input type="text" value={domain} onChange={(e) => setDomain(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && lookup()} placeholder="example.com" className="flex-1 bg-white border border-gray-300 rounded-lg px-4 py-3 text-gray-900 font-mono focus:ring-2 focus:ring-brand-500 focus:border-transparent" />
              <button onClick={() => lookup()} disabled={loading || !domain.trim()} className="px-6 py-3 bg-gradient-to-r from-brand-600 to-accent-600 text-white rounded-lg font-semibold hover:opacity-90 disabled:opacity-50 flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 mt-3 flex-wrap">
              ...
              {SAMPLE_DOMAINS.map(s => (
                <button key={s} onClick={() => lookup(s)} className="text-xs px-2 py-1 bg-gray-100 hover:bg-brand-50 text-gray-600 hover:text-brand-600 rounded transition-colors font-mono">{s}</button>
              ))}
            </div>
          ...
          {data && (
            <>
                      ...
                      return (
                        <div key={i} className={`rounded-lg p-4 border group ${isKSK ? 'bg-blue-50 border-blue-200' : 'bg-gray-50 border-gray-200'}`}>
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <button onClick={() => copy(k, `key-${i}`)} className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-brand-600">
                  ...
                  <div className="space-y-2">
                    {data.dsRecords.map((d: string, i: number) => (
                      <div key={i} className="bg-gray-50 rounded-lg px-4 py-3 border border-gray-200 flex items-center justify-between group">
                        ...
                        <button onClick={() => copy(d, `ds-${i}`)} className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-brand-600 ml-2 flex-shrink-0">
              ...
              {(!data.dnskeys || data.dnskeys.length === 0) && (!data.dsRecords || data.dsRecords.length === 0) && (
                <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6 text-center text-gray-500 text-sm">No DNSKEY or DS records found for this domain.</div>
              )}
            </>
          )}
