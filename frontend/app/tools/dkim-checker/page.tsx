      ...
      <div className="bg-gradient-to-r from-brand-600 to-accent-600 py-12">
        <div className="container-custom">
          ...
          <div className="text-center">
            <div className="flex items-center justify-center mb-6"><div className="w-20 h-20 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center shadow-lg"><KeyRound className="w-10 h-10 text-white" /></div></div>
            <h1 className="text-4xl md:text-5xl font-bold mb-4 text-white">DKIM <span className="text-blue-100">Checker</span></h1>
            <p className="text-xl text-blue-100 max-w-2xl mx-auto">Verify DKIM records with tag breakdown and selector scan</p>
          </div>
        </div>
      </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                {COMMON_SELECTORS.map(s => (
                  <button key={s} onClick={() => setSelector(s)} className={`px-2.5 py-1 rounded-lg text-xs transition-colors ${selector === s ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{s}</button>
                ))}
              </div>
            </div>
            <div className="flex gap-3">
              <button onClick={() => check()} disabled={loading || !domain.trim()} className="flex-1 py-3 bg-brand-600 hover:bg-brand-700 text-white rounded-lg font-semibold disabled:opacity-50 flex items-center justify-center gap-2 transition-colors">
