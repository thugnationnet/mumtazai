      ...
      <div className="relative py-14 md:py-20 overflow-hidden rounded-b-[2rem] themed-section-bg">
        ...
        <div className="container-custom relative z-10">
          ...
          <div className="text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-bold mb-4 font-black bg-gradient-to-r from-slate-800 via-purple-800 to-indigo-700 bg-clip-text text-transparent">Password <span className="text-slate-500">Generator</span></h1>
            <p className="text-xl text-slate-500 max-w-2xl mx-auto">Generate passwords, passphrases & PINs with entropy analysis and crack-time estimation</p>
          </div>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="bg-white/40 backdrop-blur-lg rounded-2xl border border-white/60 shadow-lg p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="flex flex-col gap-2">
                <button onClick={generate} className="p-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-colors" title="Generate"><RefreshCw className="w-5 h-5" /></button>
          ...
          <div className="bg-white/40 backdrop-blur-lg rounded-2xl border border-white/60 shadow-lg p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
              {presets.map((p, i) => (
                <button key={i} onClick={() => { applyPreset(p); }} className="p-3 rounded-xl border border-gray-200 hover:border-blue-300 hover:bg-blue-50 transition-all text-left">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white/40 backdrop-blur-lg rounded-2xl border border-white/60 shadow-lg p-6 space-y-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {([
                    ...
                    <button key={m.id} onClick={() => { setMode(m.id); setLength(m.id === 'pin' ? 6 : m.id === 'passphrase' ? 4 : 16) }} className={`p-3 rounded-xl text-center transition-all ${mode === m.id ? 'ring-2 ring-blue-500 bg-blue-50' : 'border border-gray-200 hover:bg-gray-50'}`}>
              ...
              <div>
                ...
                <input type="range" min={mode === 'pin' ? 4 : mode === 'passphrase' ? 2 : 4} max={mode === 'pin' ? 12 : mode === 'passphrase' ? 10 : 128} value={length} onChange={(e) => setLength(parseInt(e.target.value))} className="w-full accent-blue-600" />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {([
                      ...
                      <label key={opt.key} className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-colors ${options[opt.key] ? 'bg-blue-50 border border-blue-200' : 'bg-gray-50 border border-gray-200'}`}>
                        <input type="checkbox" checked={options[opt.key]} onChange={(e) => setOptions(prev => ({ ...prev, [opt.key]: e.target.checked }))} className="rounded text-blue-600 focus:ring-blue-500" />
                        <div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="flex gap-2">
                      {['-', '_', '.', ' ', ''].map(s => (
                        <button key={s || 'none'} onClick={() => setSeparator(s)} className={`px-3 py-1.5 rounded-lg text-sm font-mono ${separator === s ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                          {s || 'none'}
                        </button>
                      ))}
                    </div>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={capitalize} onChange={(e) => setCapitalize(e.target.checked)} className="rounded text-blue-600" />
              ...
              {mode === 'password' && (
                <div>
                  <button onClick={() => setShowAdvanced(!showAdvanced)} className="text-sm text-blue-600 hover:text-blue-700 font-medium">
                    {showAdvanced ? '▼' : '▶'} Advanced Options
                  </button>
                  {showAdvanced && (
                    <div className="mt-3 space-y-3 bg-gray-50 rounded-lg p-4">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" checked={options.excludeSimilar} onChange={(e) => setOptions(prev => ({ ...prev, excludeSimilar: e.target.checked }))} className="rounded text-blue-600" />
                        <span className="text-sm text-gray-700">Exclude similar chars <span className="font-mono text-gray-400">(l, 1, I, 0, O)</span></span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" checked={options.excludeAmbiguous} onChange={(e) => setOptions(prev => ({ ...prev, excludeAmbiguous: e.target.checked }))} className="rounded text-blue-600" />
              ...
              <button onClick={generate} className="w-full px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:opacity-90 text-white rounded-xl transition-opacity font-semibold flex items-center justify-center gap-2">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white/40 backdrop-blur-lg rounded-2xl border border-white/60 shadow-lg p-5">
                <h3 className="text-sm font-semibold text-gray-900 mb-2 flex items-center gap-2"><Shield className="w-4 h-4 text-blue-600" />Browser-Only Security</h3>
