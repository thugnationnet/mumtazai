      ...
      <div className="bg-gradient-to-r from-brand-600 to-accent-600 py-12">
        <div className="container-custom">
          ...
          <div className="text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-bold mb-4 text-white">Number Base <span className="text-blue-100">Converter</span></h1>
            <p className="text-xl text-blue-100 max-w-2xl mx-auto">Convert between text and Binary, Hex, Octal, Decimal & Base64 with bit visualization</p>
          </div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6">
            <div className="flex flex-wrap gap-3 items-center">
              <div className="flex gap-2">
                {modes.map(m => (
                  <button key={m.id} onClick={() => setMode(m.id)} className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${mode === m.id ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
          ...
          {direction === 'encode' && input && (
            <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6">
              <button onClick={() => setShowAllFormats(!showAllFormats)} className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                <Braces className="w-4 h-4 text-brand-600" />{showAllFormats ? '▼' : '▶'} All Formats Simultaneously
              </button>
              {showAllFormats && allFormats && (
                <div className="mt-4 space-y-3">
                  {allFormats.map(f => (
                    <div key={f.id} className={`rounded-xl p-4 border ${f.id === mode ? 'bg-brand-50 border-brand-200' : 'bg-gray-50 border-gray-200'}`}>
          ...
          {input && (direction === 'encode' ? input : output) && (
            <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6">
              <button onClick={() => setShowBitView(!showBitView)} className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                <BarChart3 className="w-4 h-4 text-brand-600" />{showBitView ? '▼' : '▶'} Byte-Level Visualization
              </button>
              {showBitView && (
                <div className="mt-4">
                  <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-2">
                    {bitView.slice(0, 60).map((b, i) => (
                      <div key={i} className="bg-gray-50 rounded-lg p-2 border border-gray-200 text-center hover:bg-brand-50 hover:border-brand-200 transition-colors">
                        ...
                        <div className="flex justify-center gap-[1px] mt-1">
                          {b.binary.split('').map((bit, j) => (
                            <div key={j} className={`w-1.5 h-3 rounded-sm ${bit === '1' ? 'bg-brand-500' : 'bg-gray-200'}`} />
                          ))}
                        </div>
