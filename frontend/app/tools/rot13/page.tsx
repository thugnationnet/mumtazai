    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-r from-brand-600 to-accent-600 py-12">
        <div className="container-custom">
          ...
          <div className="text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-bold mb-4 text-white">Cipher <span className="text-blue-100">Toolkit</span></h1>
            <p className="text-xl text-blue-100 max-w-2xl mx-auto">ROT13, ROT47, Caesar, Atbash, Vigenère & Substitution ciphers with frequency analysis</p>
          </div>
        </div>
      </div>
      ...
      <main className="container-custom py-8">
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
              {(Object.keys(cipherInfo) as CipherMode[]).map(m => (
                <button key={m} onClick={() => setMode(m)} className={`p-3 rounded-xl text-center transition-all ${mode === m ? 'ring-2 ring-brand-500 bg-brand-50' : 'border border-gray-200 hover:bg-gray-50'}`}>
          ...
          {/* Controls row */}
          <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6 flex flex-wrap gap-4 items-end">
            {!isSelfInverse && (
            ...
            {mode === 'caesar' && (
              <div className="flex items-center gap-2">
                ...
                <input type="range" min={1} max={25} value={shift} onChange={(e) => setShift(parseInt(e.target.value))} className="w-32 accent-brand-600" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6">
              ...
              {output && (
                <button onClick={() => { setInput(output) }} className="mt-2 text-xs text-brand-600 hover:text-brand-700 flex items-center gap-1"><RefreshCw className="w-3 h-3" />Use output as input</button>
              )}
            </div>
          </div>
          ...
          {input && (
            <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6">
              <button onClick={() => setShowFreq(!showFreq)} className="flex items-center gap-2 text-sm font-semibold text-gray-700">
                <BarChart3 className="w-4 h-4 text-brand-600" />{showFreq ? '▼' : '▶'} Frequency Analysis
              </button>
              {showFreq && (
                        ...
                        {side.data.map(d => (
                          <div key={d.char} className="flex items-center gap-2 text-xs">
                            ...
                            <div className="flex-1 bg-gray-100 rounded-full h-3 overflow-hidden">
                              <div className="h-3 bg-brand-500 rounded-full transition-all" style={{ width: `${d.percent}%` }} />
                            </div>
          ...
          {mode === 'caesar' && input && (
            <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6">
              <button onClick={() => setShowBruteForce(!showBruteForce)} className="flex items-center gap-2 text-sm font-semibold text-gray-700">
                <Shuffle className="w-4 h-4 text-brand-600" />{showBruteForce ? '▼' : '▶'} Brute Force All 25 Shifts
              </button>
              {showBruteForce && (
                <div className="mt-3 space-y-1 max-h-[400px] overflow-y-auto">
                  {bruteForce.map(b => (
                    <div key={b.shift} className={`flex items-center gap-3 py-2 px-3 rounded-lg text-sm ${b.shift === shift ? 'bg-brand-50 border border-brand-200' : 'hover:bg-gray-50'}`}>
