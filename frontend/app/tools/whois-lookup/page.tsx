    ...
    <div className="min-h-screen themed-section-bg">
      ...
      <header className="relative py-14 md:py-20 overflow-hidden rounded-b-[2rem] themed-section-bg">
        ...
        <div className="container-custom relative z-10 py-8">
          <Link href="/tools" className="inline-flex items-center gap-2 text-purple-600 hover:text-purple-500 mb-6 transition-colors">
          ...
          <div className="text-center">
            ...
            <h1 className="text-4xl font-bold mb-2 font-black bg-gradient-to-r from-slate-800 via-purple-800 to-indigo-700 bg-clip-text text-transparent">WHOIS <span className="text-sky-500">Lookup</span></h1>
            <p className="text-lg text-slate-500">Domain, IP &amp; DNS record lookups with history and export</p>
          </div>
          </div>
        </div>
      </header>
          ...
          <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {['created', 'expires', 'updated'].filter(k => (domainResult.dates as Record<string, string>)[k]).map(key => (
                <div key={key} className="bg-white/40 backdrop-blur-lg rounded-2xl border border-white/60 shadow-lg p-6">
                  <div className="flex items-center gap-3 mb-3">
        ...
        {activeTab === 'dns' && dnsResult && (
          <div className="max-w-5xl mx-auto space-y-6">
            ...
            {Array.from(new Set(dnsResult.records.map(r => r.type))).map(type => (
              <div key={type} className="bg-white/40 backdrop-blur-lg rounded-2xl border border-white/60 shadow-lg p-6">
        ...
        {activeTab === 'history' && (
          <div className="max-w-4xl mx-auto space-y-3">
            {queryHistory.length === 0 ? (
              ...
              queryHistory.map(q => (
                <div key={q.id} className="bg-white/40 backdrop-blur-lg rounded-2xl border border-white/60 shadow-lg p-4 flex items-center justify-between">
