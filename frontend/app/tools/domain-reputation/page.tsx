      ...
      <div className="bg-gradient-to-r from-brand-600 to-accent-600">
        <div className="container mx-auto px-4 py-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold text-white">Domain Reputation Scanner</h1>
              <p className="text-blue-100 mt-1">Deep security analysis, threat detection &amp; reputation scoring</p>
            </div>
          </div>
        </div>
      </div>
                ...
                <div className={`bg-gradient-to-r ${riskColors[report.riskLevel] || riskColors.medium} rounded-2xl p-8 shadow-xl`}>
                  <div className="flex flex-col md:flex-row items-center gap-8">
                    ...
                    <div className="text-center md:text-left flex-1">
                      <h2 className="text-2xl font-bold text-white mb-1">{report.domain}</h2>
          ...
          <div className="space-y-4">
            {state.scanHistory.length === 0 ? (
              ...
              state.scanHistory.map(record => (
                <div key={record.id} className="bg-white rounded-xl p-4 border border-gray-200 shadow-lg flex items-center justify-between">
          ...
          <div className="space-y-4">
            {state.alerts.length === 0 ? (
              ...
              state.alerts.map(alert => (
                <div key={alert.id} className={`bg-white rounded-xl p-4 border shadow-lg ${alert.severity === 'high' ? 'border-red-200' : alert.severity === 'medium' ? 'border-yellow-200' : 'border-gray-200'}`}>
