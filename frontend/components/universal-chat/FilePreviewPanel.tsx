...
export default function FilePreviewPanel({ fileOp, onOpenFile }: FilePreviewPanelProps) {
  ...
  return (
    <div className="my-3 rounded-xl border border-white/80 overflow-hidden shadow-sm bg-[#0d1117]">
      {/* ── Header bar ── */}
      <div
        className="flex items-center justify-between px-3 py-2.5 cursor-pointer select-none bg-[#161b22] border-b border-white/10 hover:bg-[#1c2129] transition-colors"
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-sm font-mono text-white/90 truncate" title={fileOp.path || fileOp.filename}>
            {fileOp.filename}
          </span>
      ...
      {expanded && (
        <div>
                ...
                <tbody>
                  {lines.map((line, i) => (
                    <tr key={i} className="hover:bg-white/5">
                      <td className="px-3 py-0 text-right text-gray-600 select-none align-top w-[1%] whitespace-nowrap border-r border-white/5">
                        {i + 1}
                      </td>
          ...
          <div className="flex items-center justify-between px-3 py-1.5 bg-[#161b22] border-t border-white/10 text-[10px] text-gray-500">
