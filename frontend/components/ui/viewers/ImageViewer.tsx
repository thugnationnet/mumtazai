          ...
          >
            ...
            {image.title && (
              <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/70 to-transparent">
                <p className="text-slate-900 text-sm truncate">{image.title}</p>
              </div>
            )}
          </div>
        ))}
    ...
    <>
      ...
      >
        ...
        {Array.isArray(src) && src.length > 1 && (
          <div className="absolute bottom-1 right-1 px-1.5 py-0.5 text-xs font-medium bg-slate-400 text-slate-900 rounded">
            +{src.length - 1}
          </div>
        )}
        <div className="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 bg-black/30 transition-opacity">
          <ArrowsPointingOutIcon className="w-6 h-6 text-slate-900" />
        </div>
      </div>
