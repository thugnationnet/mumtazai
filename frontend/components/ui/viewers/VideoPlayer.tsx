  ...
  return (
    ...
    >
      ...
      {onClose && (
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-20 p-2 rounded-full bg-slate-400 hover:bg-black/70 transition-colors"
        >
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 pb-4 bg-gradient-to-t from-black/70 to-transparent">
            <div className="flex items-center space-x-3">
              ...
              >
                {playing ? (
                  <PauseIcon className="w-6 h-6 text-slate-900" />
                ) : (
              ...
              <button
                ...
                title="Skip back 10s"
              >
                <BackwardIcon className="w-5 h-5 text-slate-900" />
              </button>
              ...
              >
                <ForwardIcon className="w-5 h-5 text-slate-900" />
              </button>
              ...
              <div className="flex items-center space-x-2 group/volume">
                ...
                >
                  {muted || volume === 0 ? (
                    <SpeakerXMarkIcon className="w-5 h-5 text-slate-900" />
                  ) : (
                    <SpeakerWaveIcon className="w-5 h-5 text-slate-900" />
                  )}
                </button>
                <input
                  type="range"
...
export function VideoThumbnail({
  ...
  return (
    ...
    >
      ...
      {/* Duration badge */}
      {duration && (
        <div className="absolute bottom-2 right-2 px-1.5 py-0.5 text-xs font-medium bg-slate-500 text-slate-900 rounded">
          {formatTime(duration)}
        </div>
      )}
      ...
      {title && (
        <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/70 to-transparent">
          <p className="text-slate-900 text-sm truncate">{title}</p>
        </div>
      )}
    </div>
  );
